...
- A fake-`launchctl` migration integration test passed: default install cleans the legacy domain and registers GUI, status/restart fall back to legacy, uninstall cleans both domains, and explicit `BOT_DOMAIN` skips migration. It also simulates delayed service disappearance after `bootout`; shutdown waits are bounded. Legacy query failures stop before bootstrap, while legacy cleanup failures roll back the new GUI service. The test uses only temporary files and does not create a second Telegram bot.
