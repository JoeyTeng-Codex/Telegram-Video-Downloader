...
- Bilibili bvid/aid matches remain duplicate prompts but cannot authorize overwrite across multipart entries. Overwrite requires one exact cid/epid match for a single-entry plan, and the move path revalidates that invariant before replacing any file.
...
- 2026-08-25 validation: `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test` (200 passed), `uv run ruff format --check`, `uv run ruff check`, and `uv run python -m unittest discover -s tests` (20 passed).
